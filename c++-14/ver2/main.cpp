...
bool strcmp_ver(char* str1, char* str2);
...
void remove_student_by_attributes(Student*& students, int& size, char* name, unsigned short age, float gpa, char* email, char* phone) {
    ...
    for (int i = 0; i < size; i++) {
        bool match = false;
        if (name != nullptr && strcmp_ver(students[i].name, name)) match = true;
        if (age != 0 && students[i].age == age) match = true;
        if (gpa != 0.0f && students[i].gpa == gpa) match = true;
        if (email != nullptr && strcmp_ver(students[i].email, email)) match = true;
        if (phone != nullptr && strcmp_ver(students[i].phone, phone)) match = true;
...
void find_student_by_attributes(Student* students, int size, char* name, unsigned short age, float gpa, char* email, char* phone ) {
        for (int i = 0; i < size; i++) {
            bool match = false;
            if (strcmp_ver(students[i].name, name) && name!=nullptr) match = true;
            if (students[i].age == age && age!=0) match = true;
            if (students[i].gpa == gpa && gpa!=0) match = true;
            if (strcmp_ver(students[i].email, email) && email!=nullptr) match = true;
            if (strcmp_ver(students[i].phone, phone) && phone!=nullptr) match = true;
            if(match == true) {
                std::cout<<"\n";
